...
\caption{This is a description of two different element types. Element A is the \PN[1]{2}, while element B is the \PN[1]{1} \red{(THIS IS NOT CORRECT!!)}. Porosity $\phi_{i}$ \red{(THIS DOES NOT APPEAR IN THE FIGURE ... AT LEAST NOT WITH THIS SYMBOL)} and permeability $K_{i}$ are CV-wise quantities, whereas velocity and pressure are primarily represented in FE space. Scalar fields (such saturation, density, viscosity etc) are also represented in CV space.}
\label{fig:fem_elem}
\end{figure}
