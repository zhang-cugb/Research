...
\caption{This is a description of two different element types. Element A is the \PN[1]{2}, while element B is the \PN[1]{1} \red{(THIS IS NOT CORRECT!!)}. Porosity $\phi_{i}$ \red{(THIS SYMBOL DOES NOT APPEAR IN THE FIGURE !!)} and permeability $K_{i}$ are CV-wise quantities, whereas velocity and pressure are primarily represented in FE space. Scalar fields (such saturation, density, viscosity etc) are also represented in CV space.}
\label{fig:fem_elem}
\end{figure}
...
\vspace{0.cm}
\hbox{\hspace{1.0cm} (a) Hell-Shaw cell domain \hspace{3.cm} (b) t=250\red{(???)} \hspace{3.0cm} (c) t=1000\red{(???)}}
\vspace{0.5cm}
\hbox{
...
\caption{Simulated flow in a Hele-Shaw cell: (a) source and sink regions are explicitly shown along with prescribed dimensions; (b-f) snapshots of wetting phase saturation showing flow profile as the simulation evolves. The domain contains \red{XX} \PN[1]{2} elements and {\it VR}=1.}
\label{fig:homoheleshaw}
\end{figure}
\end{landscape}
\clearpage
