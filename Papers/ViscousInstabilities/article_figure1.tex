...
\begin{figure}[ht] 
\vbox{\vspace{-1cm}
\hbox{\includegraphics[width=.9\textwidth, height=0.5\textwidth]{./Pics1/Saffman_homogeneous_VR10/ST_Homog_VR10_D201c}
\hspace{0.5cm}      
...
\caption{Simulated flow in a Hele-Shaw cell for viscosity ratio of 10 (left) and 150 (right \red{to be added}). \blue{ Jeff: Add comments here wrt to  tip width as a model validation wrt Saffmann's and  Guan's papers.} The domain contains $26313$ \PN[1]{2} triangular elements.}
...
\caption{Simulated flow in a modified Hele-Shaw cell with (a) distribution of permeability ({\bf K}) field $\left(\text{10}^{-10}\le\mathbf{K}_{1}\le\text{5}\times\text{10}^{-10}\right.$, {\bf K}$_{2}$=10$^{-10}$, 10$^{-11}\le\mathbf{K}_{3}\le$ 5$\times$10$^{-10}$ and 10$^{-12}\le\mathbf{K}_{4}\le$ 5$\times$10$\left.^{-10}\text{ cm}^{2}\right)$; (b-f) snapshots of saturation profile during \red{XX} seconds of simulation. Saturation map is the same as used in Fig.\red{XX ... is it true??}. The domain contains \red{XXXX} elements.}
