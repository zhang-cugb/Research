...
%\caption{\red{Impact of mesh resolution on capturing flow instabilities: snapshots of saturation field through 1.50 seconds of numerical simulation performed with fixed mesh (of 586 elements)} and {\it VR}=10.}
%\label{fig:5regions_fixedmesh_coarse}
%\end{figure}
%\clearpage
...
\caption{3D channel flow: permeability distribution. The domain contains 243056 \PN[1]{1} element-pairs.}
...
\caption{3D channel flow: XY and YZ planes showing wetting fluid phase Darcy velocity vectors overlapped with wetting fluid saturation at (a) t=$0.29$s and (b)t=$4.3$s. \red{(AS I MENTIONED BEFORE THE FIGURE SHOULD SHOW OVERLAPPING VELOCITY VECTORS ON FLUID SATURATION FIELD INSTEAD OF OVERLAPPING VELOCITY VECTORS ON VELOCITY FIELD!!!! PLEASE CHANGE IT!! }}
...
\vbox{
\hbox{\vspace{-5.0cm}
\includegraphics[width=0.75\textwidth]{./Pics1/3D_Channel/3D_channel_sat_30.pdf}
\includegraphics[width=0.75\textwidth]{./Pics1/3D_Channel/3D_channel_sat_100.pdf} 
}
\hspace{-5.0cm}
\hbox{\hspace{6.0cm} (a) \hspace{6.cm} (b)    
}
\hbox{\vspace{0.0cm}
\includegraphics[width=0.75\textwidth]{./Pics1/3D_Channel/3D_channel_sat_150.pdf}
\includegraphics[width=0.75\textwidth]{./Pics1/3D_Channel/3D_channel_sat_490.pdf}
}
\hspace{-5.0cm}
\hbox{\hspace{6.0cm} (c) \hspace{6.cm} (d)     
}
}     
\caption{The figures (a) t=$0.91$s and (b)t=$4.3$s give a description of the saturation front. }
...
\caption{3D channel flow: saturation front evolving in time and space with preferential flow pathways through 3.50 seconds of numerical simulations. Preferential flow pathway can be readily noticed in these frames, mirroring permeability distribution. The domain contains 368513 \PN[1]{1} elements.}
\label{fig:3DChannel_sat}
\end{figure}
\end{landscape}
\clearpage
...
\caption{3D channel flow: Isosurfaces for wetted phase saturatioh ranging from 0.45 and 0.60 at the same instants of time of Fig.~\ref{fig:3DChannel_sat}. Preferential flow pathway can be readily noticed in (b) and (f). Fingers' formation and growth can be clearly noticed in (b)-(e). The domain contains 368513 \PN[1]{1} elements.}
...
\caption{This is a qualitative comparison at t=end of simulation, for bot the fixed and unstructured mesh used in the 3D domain. \red{perhaps we need to change the order between this figure and the \ref{fig:3DChannel_sat_adapt}? } }
\end{figure}
%\end{landscape}
\clearpage
