...
\KCnote{Please reorganise the structure like the papers you read, and reference them!}
...
% are based on domains that require high resolution static meshes to capture the development of the fingers 
...
The remainder of the paper is orgranised as follows.
 
\KCnote{ this needs to be rephrased later -- Next section \citet{math-model} illustrates the model and the governing equations of immiscible and incompressible flow in porous media. The method and numerical formulations that describes the CVFEM follows, along with cases that demonstrate how the mesh adaptivity for simulation of viscous fingering is implemented. In the end, conclusions and final remarks of the work are discussed.--}          
 

\section{Mathematical Formulation}\label{section:math-model}
%\medskip
%\begin{comment}
...
The first example is a simple comparison between two cases using fixed mesh and mobility ratios of $1$ and $10$ respectively. The aspect ratio of the $2D$ domain is 2 and it favors viscous fingering effects while temperature is not taken into account (or isothermal) and gravity effects are negligible. Fluid $1$ is initially injected from the left hand side with velocity $u_{inj}=1$. At the outlet -right hand side- boundary pressure levels are set to zero while no slip boundary conditions are implied at the upper and lower part of the formation. Fixed time step sizes of $1e-3$ for both the fixed meshes and the CFL number for this set of simulations is 5. The $p_{1}DGP_{2}$ element type with piecewise linear description of veleocity within each element and quadratic pressure representation as part of the OCVFEM method and the discontinuous Galerkin method which composing the mesh. Saturation is located at the pressure nodes. Five different regions of 5 distinct absolute permeabilities (K) are introduced (fig.\ref{fig:5regions_domain}). The square in the initially contains a $50\%-50\%$ mixture of both phase $1$ and $2$. 

Two general behaviors for the interface may occur: (i) If a more viscous fluid displaces a less viscous one or a less viscous displaces a more viscous with a low average velocity, the interface is flat; (ii) if a less viscous fluid displaces a more viscous one with intermediate values of the average velocity, a single stable finger is observed.

This example serves as a base case for comparison between mobility (or viscocity) ratios that differ by an order of magnitude. Viscocity is a measure of resistance, therefore a frictional property. In general for mobility ratios below $5$ no fingers are expexted as the injection fron will uniformly develop as time progress. Instabilities will start to occur for mobility ratios higher than $5$. Fingers or accelerated flows will be triggered from permeability differences on the top layers of the domain and the front will start to collapse. The interface between the two fluid will become unstable, an uneven or fingered profile will be controlled by capilary pressure -acting in the scale of the pores- and therefore due to the surface tension difference between the two immiscible fluids. Concequantly the productivity of an oil field will be affected and will decrease.


\begin{figure}[h]
%\begin{center}
\includegraphics[width=0.75\textwidth]{./Pics1/mr1_fixed/permeability.png}
\centering
\caption{2D domain that consist of five regions with 5 different absolute permeabilities.}
...
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_fixed/mr1_fixed_100.png}
}%
\subfigure[MR10 fixed mesh start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_fixed/mr10_fixed_100.png}
}\\
\subfigure[MR1 fixed mesh middle]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_fixed/mr1_fixed_middle.png}
}%
\subfigure[MR10 fixed mesh middle]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_fixed/mr10_fixed_middle.png}
}\\
\subfigure[MR1 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_fixed/mr1_fixed_end_1.png}
}\\
\subfigure[MR10 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_fixed/mr10_fixed_end_1.png}
}\\
\end{center}
\caption{Comparing MR1 and MR10 cases using fixed mesh. Simulation time step increases from top to bottom and from t=100(start) to t=1500(middle) to t=3000(end).}
\label{fig:}
\end{figure}

The next set of simulations is a comparison between a fixed and adapt mesh for the MR=$10$.
...
\subfigure[MR1 fixed mesh start]{%
\label{fig:first}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_adapt/mr1_adapt_100.png}
}%
\subfigure[MR10 fixed mesh start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_adapt/mr10_adapt_10.png}
}\\
\subfigure[MR1 fixed mesh 500]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_adapt/mr1_adapt_500.png}
}%
\subfigure[MR10 fixed mesh 500]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_adapt/mr10_adapt_50.png}
}\\
\subfigure[MR1 fixed mesh half]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_adapt/mr1_adapt_1500.png}
}%
\subfigure[MR10 fixed mesh half]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_adapt/mr10_adapt_150.png}
}\\ 
\subfigure[MR1 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr1_adapt/mr1_adapt_end.png}
}\\
\subfigure[MR10 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_adapt/mr10_adapt_end.png}
}\\
\end{center}
...
\subfigure[MR1 fixed mesh start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_fixed/5regions_fixed_start.png}
}%
\subfigure[MR10 fixed mesh start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_adapt/5regions_adapt_start.png}
}\\
\subfigure[MR1 fixed mesh 500]{%
\label{fig:}
...
\subfigure[MR10 fixed mesh half]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_adapt/5regions_adapt_1500_1.png}
}\\ 
\subfigure[MR1 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_fixed/5regions_fixed_end.png}
}\\
\subfigure[MR10 fixed mesh end]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_adapt/5regions_adapt_end.png}
}\\
\end{center}
\caption{Comparing the  MR10 cases -fixed and adaptive mesh- for the same time steps}
\label{fig:}
\end{figure}
...
\subfigure[MR1 fixed mesh start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_5regions_fixed_dinlet/5regions_dinlet_fixed_start.png}
}%
\subfigure[MR10 fixed mesh start]{%
\label{fig:}
...
\end{center}
\caption{Comparing the  MR10 cases with double inlet -fixed and adaptive mesh- for the same time steps}
\label{fig:}
\end{figure}
...
\subfigure[start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only/Four_regions_python_only_start.png}
}%
\subfigure[start]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only_adapt/Four_regions_python_only_adapt_start.png}
}\\
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only/Four_regions_python_only_2000.png}
}%
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only_adapt/Four_regions_python_only_adapt_2000.png}
}\\
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only/Four_regions_python_only_4000.png}
}%
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only_adapt/Four_regions_python_only_adapt_4000.png}
}\\ 
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only/Four_regions_python_only_end.png}
}\\
\subfigure[]{%
\label{fig:}
\includegraphics[width=0.35\textwidth]{./Pics1/mr10_4regions_python_only_adapt/Four_regions_python_only_adapt_end.png}
}\\
\end{center}
\caption{Comparing the  MR10 cases for $4$ regions using python to define the domain characteristics for -fixed and adaptive mesh- for the same time steps}
\label{fig:}
\end{figure}
