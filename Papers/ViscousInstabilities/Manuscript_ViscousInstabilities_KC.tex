...
%\medskip
Under certain conditions the displacement of fluids in porous media is unstable \red{(what sort of conditions?)} and results in the formation of strange patterns \red{(sentence is not very technical} in the form of \textit{fingers}. This phenomenon is referred to as \textit{viscous fingering}. The flow of an injected fluid into a geological formation (porous medium) generates instabilities due to the heterogeneities of the porous medium \red{(Is this correct? Is it your opnion? If so built up the facts to substantiate your conclusions. If it is not your opinion then give reference(s))}. Even though the mechanism of the instabilities may vary (in immiscible fluids the most important factor is the surface tension while in miscible fluids, it is the diffusion \red{(thermal or mass diffusion ...be specific)}), however it is still a function of several variables such as, relative mobility, gravity, dispersion, permeability differences. Moreover, in most cases interfacial tension is directly linked with the mobility ratio (or viscosity ratio) between the displaced and the displacing fluid.

Both the evolution of the computers and the advances in modelling and simulation of flows in porous media may help to visualise and address the importance of this phenomenon which can be found in many petroleum, chemical and environmental related applications. \red{(sentence is not written in a technical format, also what is its objective?)} 
...
In this paper, the overlapping control volume finite element method (OCVFEM), formulation recently introduced by~\citet{jackson_2013}~\citep[see also][]{gomes_2013,pavlidis_2013} is used to investigate the development of multiphase flows through heterogeneous porous media \red{(what is the objective of this paper? To study multiphase porous media flows? Or to study a particular physical aspect of multiphase flows in porous media? You need to be precise here! )}. In the OCVFEM, a dual pressure-velocity representation in CV and FEM spaces  are embedded into a family of triangular/tetrahedral element pairs, P$_{m}$DG-P$_{n}$~\citep{cotter_2009b} and P$_{m}$DG-P$_{n}DG$~\citep{xie_2014}. In these element types, the dual velocity and pressure fields are represented by $m^{\rm th}$- (discontinuous) and $({n})^{\rm th}$-order (continuous/discontinuous) polynomials, respectively.
