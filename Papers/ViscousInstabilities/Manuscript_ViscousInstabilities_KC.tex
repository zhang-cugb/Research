...
The purpose of this paper is to numerically investigate the dynamics of viscous flow instabilities in porous media. Simulations involving formation and growth of viscous fingers were conducted with the CVFEM-based multi-fluid flow simulator Fluidity/IC-FERST model. The model is based upon a dual consistent pressure-velocity representation in CV and FEM spaces and a novel family of FE-pairs, \PN[n]{m} and \PNDG[n]{m}. Saturation and other scalar fields are discretised in CV space and may be fully discontinuous across finite element boundaries. %Material absolute permeability is piecewise constant in FE space to ensure the correct representation of the surface.
In order to strongly enforce discretised Darcy equations at the boundaries between elements and control volumes, a directional-weighted flux-limited was introduced to take into account discontinuities risen from both, numerical formulation (based on discontinuous-Galerkin finite element method, DGFEM) and control volumes within finite elements.


During immiscible multi-fluid flow displacements, the expected uniform front is perturbed if fluids have different densitites and/or viscositiesleading to an uneven front with elongations at the outside edge of the fluid interface 


viscous fingering occurs when the viscosity ratio is greater than unity. As surface tension becomes weak, the interface is stressed and becomes unstable leading to the formation of fingers.
