...
The last of the simulations is related with the effort to describe the finger evolution and the idea of the prefereable path using 3D domain with different permeability regions as these are decribed by figure \ref{fig:3DChannel_PermMesh}.   

\blue{(b) set up what are the main issues (theory) of the simulations you are showing in this section;}
\textit{Viscous fingering} is characterized by the formation of a characteristic pattern, called
viscous fingers. The geometrical evolution of these patterns provides good indications about the evolution of the penetration into a zone with a less viscous fluid. 

\blue{(c) then briefly describe what are the strategies to tackle these issues; }
Capturing and tracking the  evolution of viscous fingers is important for the understanding the penetration process and fluid patterns that will be created.  




\blue{(d) then describe the numerical experiment w all BC and IC; (e) then finally describe the results with a clear conclusion based on observation but backed by theory and/or physics!!}
