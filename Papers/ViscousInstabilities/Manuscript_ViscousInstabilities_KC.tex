...
\medskip
%Permeability values have been prescribed in the formation following a simple quadrant heterogenous model that will give the cross-flow that is expected in such a system. 
The flow profile follows the same behaviour with the experimental work of \citet{evans_1994}. In both cases the flow will not follow the standar source-sink behaviour while the mobility ratio (or VR) is playing a crucial role for these flows. The main difference between these two works is that by introducing an unstructured mesh the flow patterns are not only verified but also there is a more accurate descriptions during the development of the flow.    
