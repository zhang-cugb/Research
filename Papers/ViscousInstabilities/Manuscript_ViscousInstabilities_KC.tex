...
\end{displaymath}
Though, whilst saturation is calculated in CV space, permeability is piecewise constant in FE space to allow efficient representation of surface parameterised geometries (element $A$ in Fig.~\ref{fig:fem_cv}b). Saturation equations (Eqn.~\ref{eqn:saturation_eqn}) are discretised in space with CV basis function and with the $\theta$-method in time as described by \citet{gomes_book_2012}. Velocities across CV interfaces (within and between elements, see Fig.\ref{fig:fem_cv}a) are calculated through a directional-weighted flux-limited scheme based on upwind value of $\sigma$ at individual CV as described by \citet{gomes_2017}.

This formulation is applicable to $\mathcal{N}_{p}$ fluid phases and is based on the novel family of \PNDG[n]{m} element-pairs~\citep{cotter_2009a} consistent with the dual pressure-velocity representation in control volume (CV). In this family of FE-pairs velocity is represented by $n^{\text{th}}$-order polynomials that are discontinuous across elements, whereas pressure is represented by $m^{\text{th}}$-order polynomials that may be either discontinuous or continuous (thus the notation \PNDG[n]{m}) across elements. The mass balance (continuity) equations are solved in CV space and a Petrov-Galerkin FEM is used to obtain the high-order fluxes on CV boundaries, which are limited to yield bounded fields~\citep{jackson_2013}. Simulations performed for this work were conducted using two types of elements: \PN[1]{2} and \PN[1]{1}. Saturation, and all saturation-dependent material properties such as relative permeability and capillary pressure, are represented CV-wise. %Hence, each CV (and FE) is associated with a unique set of petrophysical properties, in contrast to most previous methods in which CVs span multiple values of petrophysical properties described FE-wise.
...
Equation~\ref{eqn:saturation_eqn} is discretized in time and space using the $\theta$-method. Summing over all phases yields the global mass balance equation %\ref{global_mass_balance},
