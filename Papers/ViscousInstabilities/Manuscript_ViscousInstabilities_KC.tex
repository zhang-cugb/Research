...
\noindent Having already verified that model is working for the hele-shaw experimental test-cases, now a more complicated case will be introduced with regions of different geometrical domains and different permeabilities. Figure \ref{fig:testcase_heter_domain} gives a description of the domain that will be used.

\subsection{\blue{need to find a name for this heterogeneus region/set of test-cases}}

\medskip
\KCnote{mpla mplam mpla the case with 4regions and permeabilities}
...
\caption{Comparing test-cases of fixed and adaptive mesh, using the $P_{1}DGP_{2}$ element type for MR=$10$ under the same time steps, t=250(a,b), t=500(c,d).,followed by the t=1500(a,b), t=3000(c,d) in fig.\ref{fig:2btestcase}.}
\label{fig:2atestcase}
\end{figure}
...
\caption{When adaptive mesh is introduce there is better repersentation of the fluid instabilities as these are developed on time. }
\label{fig:2btestcase}
\end{figure}
