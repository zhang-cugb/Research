...
\medskip
In Sections \ref{section:results:setup}-\ref{section:results_hete_fix_adapt}, a $2$D domain was used to describe one or more finger formations. These flow instabilities were triggerded from the permeability differences in the domain. In $3D$ domain, difference between a flow instability and the preferential flow path that may include smaller fingers is more obvious \red{(I AM NOT SURE IF I UNDERSTAND THIS SENTENCE ... YOU MAY WANT TO IMPROVE IT !!)}. Figure \ref{fig:3DChannel_sat} describes the flow path in various instances of time showing the progression of the saturation front that now becomes more obvious \red{(TRY TO AVOID AS MUCH AS YOU CAN THE USE OF STRONG WORDS IN TECHNICAL WRITING ... SUCH AS 'OBVIOUS', 'GREAT', ' POWERFUL' ETC)}. Heterogeneity is determine by the spatial distribution of porosity ($\phi =0.20$ in the current case) and permeability. Permeability depends on the number, size and connectivity of the pores. An on purpose implementation of significant high permeability zones (compared with the lower ones) also know as higher permeability channels, allows the domain to give a good approximation of the so called "thief zones". \red{(THIS LAST SENTENCE ('Permeability depends on ...') IS QUITE CONFUSING AND I AM NOT SURE WHAT YOU WANT TO SAY  OR ITS AIM/OBJECTIVE ...  TRY TO REWRITE IT ... IS THIS A COMMON NAME -- 'THIEF ZONE', IN THE LITERATURE ? IF SO, GIVE REFERENCE).}

\medskip 
The progress of the saturation front and formation and growth of fingers in time and space can be seen in more details in isosurfaces shown in Fig.~\ref{fig:3DChannel_Isosurf}. However due to relatively low mesh resolution, in particular above and below the central channel region, capture of fingers (formation and growth) was quite limited
