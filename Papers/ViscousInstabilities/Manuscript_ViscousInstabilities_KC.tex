...
\begin{abstract} 
We examine the flow instabilities, \ie viscous fingering on the displacement of one fluid by a second, using a novel computational method for modeling multi-phase flows in heterogeneous porous media. Those "fingers" will multiply and develop further into complex networks that will eventually effect the efficiency of field operations.    

Starting from modelling the Hele-Shaw experimental apparatus, a mathematically analogous process to the two-dimensional flow in porous media problem, the finger development as a function of mobility ratio (MR), different domain set-up under uniform or different permeabilities regions and mesh types is examined. A series of cases are listed, showing the validation of the method against experimental setup followed by a more complex 2D domain setup under different viscosity ratios and different mesh resolutions. Finally, a 3D domain setup is showcasing the difference between the preferential flow path and the fingering regimes that will occur in the domain. 
...
%Flow instabilities will occur due to: pressure gradient along the interface of fluids; stress forces acting in opposite directions; mobility differences that may exceed critical limits and; heterogeneities of the domain (layering). 
When viscous force of the injected fluid overcomes the capillary force, hydrodynamic instabilities may occur, resulting in the collapse of the interface between fluids and start forming fingers. 
...
\medskip
In immiscible displacements, viscous fingering occurs when the viscosity ratio is greater than unity. As surface tension becomes weak, the interface is stressed and becomes unstable leading to the formation of fingers. At this point it should be mentioned that there are two parameters, the Peclet number (Pe) and mobility ratio (M), that determine the stability characteristics. There are always a few dominant fingers that spread and shield the growth of other fingers. The interface of the main finger collapses and starts splitting into new lobes of fingers. One of these new fingers may eventually outgrow the others and then spreads to occupy an increasingly larger width. In the process, the finger reaches a critical width while the concentration gradient of its front becomes steep as a result of stretching caused by the cross-flow; thus causing the tip of the finger to become unstable and splits again, and the pattern repeats itself. Therefore, surface tension plays an essential dual role, it must be weak enough for the tip front to be unstable, but it is also the physical force causing the spreading and ensuing repeated branching \citep{tan_1988, carvalho_2013}.

\medskip
In the next section \ref{section:results_setup} is an introduction to the model that will be used followed by section \ref{section:results_initial_model_validation} will be discussed. Multi-fluid simulations are performed to numerically investigate (a) the impact of heterogeneity (here represented by spatially-dependent permeability) on the onset and propagation of viscous instabilities (Sections \ref{section:results_homo_hete}-\ref{section:results_hete_fix_adapt}) (b) flow pathways in 3D heterogeneous systems (Section \ref{section:results_3D}). \red{KOSTAS, please read and rewrite this paragraph!}
...
A two-phase immiscible flow along homogeneous and parallel layers of contrasting petrophysical properties such as porosity and permeability is initially simulated. During the displacement
of one phase by another, fluids may crossflow between adjacent layers, due to any combination of the viscous, capillary and/or gravitational forces which drive multiphase flow. In this set of simulations, crossflow caused by viscous forces, which is commonly termed viscous
crossflow is investigated. Figures~\ref{fem_cv_represent_a}(b) and (c) show the continuous displacement of fluid 2 due to the injection of fluid 1. They also demonstrate the preferential flow path through high-permeability regions. Such flow behaviour, represented by the crossflow through the four regions, is a good qualitative agreement with experiments conducted by \citet{dawe_2008}. 
...
\red{(1)Short (1 paragraph) summary of the paper with focus on the main theoretical aspects that were proved by our work in the present manuscript;}
The purpose of this paper is to describe the fluid instabilities, in particular the viscous fingering, and demonstrate a new computational approach used in order to capture the finger development. Starting from validating the development of fingers in a computational domain against experimental setup a series of other computational cases is showed focusing on the details of the flows and the introduction of parameteres by avoiding any upscaling requirements.    

\red{(2)Summary (1-3 paragraphs) of the main results with the appropriate analysis (\ie how you work has proved the theory);}
\medskip
...
\begin{abstract} 
\red{(1) Motivation (1-2 sentences);}
We examine the flow instabilities, \ie viscous fingering on the displacement of one fluid by a second, using a novel computational method for modeling multi-phase flows in heterogeneous porous media. 

\red{(2) Main aim of the manuscript highlighting the novelty of the work (you must not use the word novelty ... this should be implicit, 2-6 sentences);}
Starting from modelling the Hele-Shaw experimental apparatus, a mathematically analogous process to the two-dimensional flow in porous media problem, the finger development as a function of mobility ratio (MR), different domain set-up under uniform or different permeabilities regions and mesh types is examined. A series of cases are listed, showing the validation of the method against experimental setup followed by a more complex 2D domain setup under different viscosity ratios and different mesh resolutions. Finally a 3D domain setup is displaying the difference between the preferential flow path and the fingering regimes that will occur in the domain. 

\red{(3) summary of the main technologies/methods (2-5 sentences)}
...
\red{(4) summary of the main findings}
It was found that viscosity ratio along with the initial and boundary conditions, as well as the use of adaptivity on top of an unstructured mesh will allow the user to directly apply and examine a geological model accurately and without the need of upscaling. Properties such as porosity and adaptivity can be easily introduced in the model while the idea of the preferential flow bath can be distinguished from the finger formation mechanics, in particular the tip-splitting finger behaviour.

\begin{enumerate}
  \item \green{Try do avoid (as much as possible) the use of \underline{{\bf 'we'}} in technical/academic writing;}
  \item \green{\underline{\bf Motivation} means:
    \begin{itemize}
      \item Why is the subject of study important?
      \item What sort of applications will this study impact upon?
    \end{itemize}}
  \item \green{\underline{\bf Main aim} means:
    \begin{itemize}
      \item What is the main goal of this study?
    \end{itemize}}
...
\medskip
A numerical simulation of the same problem (\ie same geometry, initial mesh, initial and boundary conditions) was also performed using adaptive mesh, Fig.~\ref{fig:3DChannel_sat_adapt}, with resolution ranging from 235242 to 643064 tetrahedral \PNDG[1]{1} element-pairs (Fig.~\ref{3DChannel_satvel_XYplanes_adapt}a). Distinct preferable flow pathways can be shown (Fig. \ref{3DChannel_satvel_XYplanes_adapt}c)
%(Fig.~\ref{fig:3DChannel_severalfields_adapt}c)
, in particular through the central channel (see streamlines in 
Fig. \ref{3DChannel_satvel_XYplanes_adapt}f)
%Fig.~\ref{fig:3DChannel_severalfields_adapt}b) 
