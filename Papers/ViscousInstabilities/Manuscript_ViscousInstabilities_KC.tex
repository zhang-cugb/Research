...
%\end{minipage}%
\caption{For the same time step, t=1000, these plots describe the velocity magnitudes of the phase $1$ (injected fluid) under the same boundary and initiall conditions. Reading from top to bottom, the velocity magnitude %for fixed mesh is plotted(top), the velocity magnitude 
for adaptive mesh (middle) and finally the velocity magnitude for adaptive mesh with double inlet (bottom) is compard. The main difference between the top plot and the middle one is the ability to capture in greater detail, the fluid instabilities as they happen during the finger development and their velocity patterns. While there is a significant difference between the top and middle plot, at the bottom one, the impact of a second injection interval can be seen from the slope and the rate that the velocity magnitude is changing. }
\label{fig:vel_magn}
\end{figure}
