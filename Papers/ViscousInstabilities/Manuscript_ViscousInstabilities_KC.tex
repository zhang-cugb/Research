...
\subsection{3-D Channel Flows: Investigation of Preferential Flow Path and Formation of Fingers} \label{section:results_dinlet}

\blue{(a) Little intro of the case; }\\
The last of the simulations is a $3$D domain that will help in the description of the finger evolution and the idea of the preferential path using different permeability regions, as these are decribed in figure \ref{fig:3DChannel_PermMesh}.   

\blue{(b) set up what are the main issues (theory) of the simulations you are showing in this section; }\\
As already mentioned \textit{viscous fingering} is characterized by the formation of a characteristic pattern, called viscous fingers. The geometrical evolution of these patterns provides good indications on capturing and tracking any discontinuities or instabilities in the flow front. 

\blue{(d) then describe the numerical experiment w all BC and IC;}\\
Figure \ref{fig:3DChannel_PermMesh} describes the permeability pattern as this is implied in a $3D$ domain. The surface on the left hand side of the domain is the area where fluid $1$ is injected with a velovity $ u=2.5 cm/sec $. In the areas around the domain a no-slip condition is implied (\ie insulation, no exchange of material or Neumann boundary condition for mass transfer) while at the right hand side of the domain is the outlet. The domain is constructed in such a way so as to be able to capture the preferencial path of flow as this can be seen from figures \ref{fig:3DChannel_planes}-\ref{fig:3DChannel_Isosurf}.

\blue{(e) then finally describe the results with a clear conclusion based on observation but backed by theory and/or physics!!}
Figure \ref{fig:3DChannel_planes} gives a comprehensive description of the contours of phase $1$ darcy velocity, in the beggining and just before the end of the simulation. 
While the saturation level for phase $1$, remains between the values of $0.25-0.75$ the main difference between the figures \ref{fig:3DChannel_planes} (a) and (b) is the intensity of the flow as this can been seen by the darcy velocity contours.  Initially most of the contours for phase $1$ are populated and achieve their higher values closer to the inlet region. At this point a comparison between the figures \ref{fig:3DChannel_planes}(a)-\ref{fig:3DChannel_sat}(a) and \ref{fig:3DChannel_planes}(b)-\ref{fig:3DChannel_sat}(f) as they both describe the darcy velocity contours and the saturation front for the same timesteps, is necesserary. 

\medskip
As the flow progress, in the $2D$ domain a finger formation will start to appear. This is triggerded initially cause of the permeability differences in the domain. In the $3D$ domain though, the same finger formation does not look the same, as it is more a description of the preferential flow path that may include smaller fingers which will break out of the flow. Figures \ref{fig:3DChannel_sat}(a)-(f) give a description of the preferable flow path in various time-steps alongside with the evolution of the saturation front that now becomes more obvious. Based on the boundary conditions that implemented, the preferencial path of the flow can been seen. In a large scale enhanced oil-recovery operation this might have direct impact on the in-situ hydrocarbons located in lower permeability areas. 

\medskip
Thus macroscopic sweep efficiency is affected by the heterogeneity in the reservoir which controls the spatial distribution of porosity and permeability. The permeability depends on the number, size and connectivity of the pores. An on purpose implementation of significant high permeability zones (compared with the lower ones) also know as higher permeability channels or layers in the set up of the model, allows our test-cases to give a good approximation of the so called "thief zones", something that is common in heterogeneous geological formations.

\medskip 
The evolution and results of the saturation front and the idea of preferential flow path in the domain is followed by a more detail representation as this can be found in figure \ref{fig:3DChannel_Isosurf}. For the same timesteps of the saturation front evolution an inside look on the domain provides with a more detailed description of not only the flow path itself as well as the minor fingers that may be created.

\medskip
At this point it is proper to review the mechanisms of viscous fingering (\ie shielding, spreading and tip-splitting, \citet{homsy_1987}). In all the previous cases, as surface tension becomes weak, the front of the steady finger is susceptible to a viscous-fingering instability. As a result of shielding, one of these fingers will eventually outgrow the other and, will then spread to occupy the appropriate width of the domain ahead of it. Following this process, it reaches a width that is again unstable and the pattern repeats. So far shielding and spreading were dominating the flow but using this this domain and conditions the mechanics of tip-splitting along a front can be understood at least qualitatively.
...
\medskip
As usually seen for high viscosity contrast, there is a leading finger (tip splitting mechanism) that breaks out of the front by suppressing its growth to the point that smaller fingers will break out driven by surface tension. At long times the large finger advances approximately at in a constant flow rate. In order to run all these simulations in detail, the use and benefits of dynamic mesh adaptivity to capture the viscous fingering in reservoir flows was introduced. Commercial simulators provide the user with option of an unstructured type of mesh still they are not able to dynamically adapt the mesh as the simulation develops and they increase the computational cost as the saturation front changes slowly.
...
\item[2.] Computational cost can be reduced because the complex geometries and surfaces are less expensive to generate and manage using fewer grid cells. The basis of the this argument is the flexibility and variance of the unstructured grid in time and space.
...
\begin{enumerate}
\item[1.] After having separate the cross flow pattern from the viscous finger one, the findings show that the displacing fluid tends to flow easier towards the zones with higher values of permeability, as these can been seen from evolution of fingers in the upper layers of the domains or from cross-section (\red{i want to say that in hele-shaw cells this can be seen from above, longitudinal section? or frame plan}) througout the Hele-shaw domains. 
