...
\begin{abstract} 
%Numerical modelling of multiphase flow in porous media has applications in a wide range of disciplines Accurate prediction of multiphase flows in porous media remains one of the problems. While the governing equations are the conservation of mass and Darcy's law, there are also the primary variables, such us pressure, velocity, saturation and permeability. This paper introduces a multi-fluid porous media flow model which is the basis of a general purpose flow simulator that is equipped with anisotropic mesh adaptivity techniques to control the local solution errors and  increase the computational accuracy in the research on the development of numerical methods in geophysical fluid mechanics. The governing multiphase porous media flow equations are solved using the adaptive and unstructured mesh.
...
Detailed numerical investigation of multiphase flows in porous media has attracted the attention of the scientific community for the past 50 years. This is mainly due to applications in oil and gas exploration and recovery, groundwater resources, underground coal gasification, geothermal power systems and, more recently, in carbon capture and storage (CCS) processes~\citep{spycher_2003, chen_2006,self_2012,pruess_1990c, white_1981,jiang_2011}. Complex geometries with irregular boundaries and strong heterogeneity in geological formations are the main issues addressed by developers of industry- and academic-standard flow simulators. 

Accurate prediction of multiphase flow behaviour through realistic porous media domains is one of the the main research challenges as \red{write about the main issues related to heterogeneity and link it with flow behaviour (2-4 sentences)}. 
...
%\medskip
Under specfic spatial conditions the displacement of fluids in porous media is unstable {\bf(why?)} and results in the formation of irregular flow patterns in the form of fingers. This phenomenon, viscous fingering, describes flow instabilities {\bf (and ...??)}. Even though the mechanism of instabilities may vary (in immiscible fluids the most important factor is the surface tension while in miscible fluids, it is the diffusion \red{(thermal or mass diffusion ...be specific)}), however it is still a function of several variables such as, relative mobility, gravity, dispersion, permeability differences. Moreover, in most cases interfacial tension is directly linked with the mobility ratio (or viscosity ratio) between the displaced and the displacing fluid.
\medskip
%Both the evolution of visualisation techniques through the extensive use of parallel computing as well as the advances in modelling and simulation of flows in porous media may help to represent and address the importance of this phenomenon which can be found in petroleum, chemical and environmental related applications. {\bf (does the paper address any aspect of parallel computation or computational/data visualization?)}

In this paper, the overlapping control volume finite element method (CVFEM) formulation, recently introduced by~\citet{jackson_2015}~\citep[see also][]{radunz_2014}, is used to investigate the development of multiphase flows through heterogeneous porous media. In this formulation, the dual pressure-velocity representation in CV and FEM spaces are embedded into a family of triangular/tetrahedral element-pairs, \PN[n]{m} and \PNDG[n]{m} \citep[][]{cotter_2009b,xie_2014} -- basis function with $n^{\rm th}$- (discontinuous) and $m^{\rm th}$-order (continuous/discontinuous) polynomials for velocity and pressure fileds, respectively.  Scalar fields (e.g., saturation, concentration, density, etc) are represented in the CV space. High-order accurate downwind schemes on element boundaries on discontinuous scalar fields are flux-limited (based on NVD approach) to obtain bounded and compressive (capturing the interfaces) solutions. This formulation can achieve an exact representation of the Darcy force balance equations.

\medskip

Here, the main objective is to investigate the impact of heterogeneous porous media in fingers formation using a FEM-based unstructured fixed mesh multi-fluid model with uniform permeability distribution under both saturated and unsaturated porous media. {\bf (try to develop it further; be more specific. Write this objective sentence/paragraph based on the paper's conclusions!)}. This study focuses on flow instabilities determined {\bf(determined or controlled?)} by the viscosity ratio.  Section $2$ summarises the numerical formulation used in this work, followed by description of the flow instabilities  and a description of model validation. Finally, conclusions are drawn in the final section. {\bf (Needs better description of the remaining of teh paper.)}
