...
The last of the simulations introduced is a $3$D domain that will showcase the idea of the preferential flow path and finger growth using different permeability regions, as these are decribed in figure \ref{fig:3DChannel_PermMesh}. In large scale geological formations preferential flow allows much faster contaminant transport and creates significant consequences for ground-water quality.   
...
\medskip
In sections \ref{section:results:setup}-\ref{section:results_hete_fix_adapt}, a $2$D domain was used to describe one or more finger formations. These flow instabilities were triggerded because of the permeability differences in the domain. Using the $3D$ domain, the difference between a flow instability and the preferential flow path that may include smaller fingers is more obvious. Figures \ref{fig:3DChannel_sat}(a)-(f) describe the flow path in various time-steps alongside with the progression of the saturation front that now becomes more obvious. Heterogeneity in the reservoir controls the spatial distribution of porosity ($\phi =0.2$ in the current case) and permeability. The permeability depends on the number, size and connectivity of the pores. An on purpose implementation of significant high permeability zones (compared with the lower ones) also know as higher permeability channels, allows the domain to give a good approximation of the so called "thief zones".
