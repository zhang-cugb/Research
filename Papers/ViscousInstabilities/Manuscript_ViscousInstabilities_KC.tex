...
\end{equation}
This expression is also know as the Young-Laplace equation, a relation describing the capillary pressure difference \red{(capillary pressure or pressure ??)} across the interface between two fluids. With $\Delta P$ denoting the pressure difference and $\hat{n}$ is the unit normal vector out of the surface. 
...
\red{QUICK QUESTIONS ... \\
\noindent
A) FOR THE FIRST TEST-CASE, WAS THE SIMULATION PERFORMED IN A SQUARED DOMAIN (FROM THE PICS, IT SEEMS TO BE RECTANGULAR)? \\ B) THUS ASSUMING THAT THE FIRST DOMAIN WAS RECTANGULAR, THUS IT IS THE SAME AS THE SECOND SIMULATION! THUS WHY THE SATURATION PROFILE IS DIFF (FOR VR=1) FROM SIMULATION 1??? \\
\noindent 
C) ALSO: DO NOT USE 'WE' IN TECHNICAL TEXT ... IT DOES REALLY DOWNGRADE IT !!! }   
  
   
%%%%%%%%%%%%  
\subsection{Flow Simulations in Hele-Shaw Cells at Several Viscosity Ratio Conditions}\label{section:results_homo_hete} 
 
In order to investigate the impact of viscosity ratio on flow behaviour, numerical simulations of fluid displacement were conducted in Hele-shaw cells (\ie parallel flat plates separated by infinitesimal space) following the work by \citet{saffman_1986}. In this manuscript, he investigated the onset of viscous flow instabilities \citep[following his seminal work in][]{saffman_1958} and the impact of boundary conditions in the problem's mathematical formulation and solutions.
 
Here, 2D simulations were conducted in a 5$\times$5 unit-length Hele-shaw cells (Fig.~\ref{fig:homoheleshaw_VN3}a) fully saturated with a fluid. Wetting phase fluid is driven from the bottom left-hand corner of the domain with velocity of $\frac{\sqrt{2}}{2}$. Newmann (\ie no flux) boundary condition was imposed to all borders of the domain except at the top right-hand corner (named as sink), and a pressure gradient between source and sink regions were initially imposed to the system. 

Figures~\ref{fig:homoheleshaw_VN3}-\ref{fig:homoheleshaw_VN10} show continuous fluid displacement in simulations conducted with {\it VR}=3 and {\it VR}=10, respectively. At relatively low viscosity ratio, saturation evolves in time with a smooth front throughout most of the domain, and no finger could be observed. Elongated saturation profile is due to pressure gradient near the sink region. However, in the simulation conducted with {\it VR}=10
\medskip
In fig.\ref{fig:1a_homoheleshaw_3}(a), there is the description of the domain. Three different cases with different set-ups will follow. The first one is the homogeneous domain where permeability $K_{1}=1.e-10$ everywhere while for the heterogeneous case the following permeabilities have been introduced, $K_{1}=(1.e-10 \;to\; 5.e-10$), $K_{2}=1.e-10$, $K_{3}=(1.e-11 \;to\; 5.e-10)$ and $K_{4}=(1.e-12 \;to\; 5.e-10)$. For the homogeneous case we use two different mobility ratios of $3$ and $10$ while we proceed to the heterogeneous case using a mobility ratio of $10$. 
...
\medskip
Viscous flow instabilities can be found across many research areas, disciplines and scales, from chemical and medical separation systems to large scale geological reservoir fields. \citet{muskat_1934} initially derives a perturbation solution of an axisymmetric problem (injected fluid between two infinite parallel plates) assuming a circle shaped front of the interface while his main interest is on the interface pressure shape and pressure for different values of capillary number ($N_{c}$) as this is defined below in eq.~\ref{eq:capillarry_number} where $\mu$ is the viscosity of the fuid that initially fills the Hele-Shaw cell,
T is the surface tension and U is a velocity characteristic of the moving interface. 
