...
The last of the simulations is related with the effort to describe the finger evolution and the idea of the prefereable path using 3D domain with different permeability regions, as these are decribed in figure \ref{fig:3DChannel_PermMesh}.   

\blue{(b) set up what are the main issues (theory) of the simulations you are showing in this section; }\\
\textit{Viscous fingering} is characterized by the formation of a characteristic pattern, called viscous fingers. The geometrical evolution of these patterns provides good indications on capturing and tracking the fluid patterns. 

\blue{(d) then describe the numerical experiment w all BC and IC;}\\
Figure \ref{fig:3DChannel_PermMesh} describes the permeability pattern as this is implied in a $3D$ domain. The surface on the left hand side of the domain is the area where fluid $1$ is injected with a velovity $ u=2.5 cm/sec $. The areas around the domain have a no-slip condition is implied (\ie insulation, no exchange of material or Neumann boundary condition for mass transfer) while at the right hand side of the domain is the outlet. The domain is constructed in such a way so as to be able tos capture the preferencial path of flow as this can be seen from figures \ref{fig:3DChannel_planes}-\ref{fig:3DChannel_Isosurf}.
...
Figure \ref{fig:3DChannel_planes} give a comprehensive description of the contours of phase $1$ darcy velocity, in the beggining and just before the end of the simulation. 
While the saturation levels for phase $1$, remain between the values of $0.25-0.75$ the main difference between the figures \ref{fig:3DChannel_planes} (a) and (b) is the intensity of the flow as this can been seen by the darcy velocity contours.  Initially most of the contours for phase $1$ are populated and achieve their higher values closer to the inlet region. At this point a comparison between the figures \ref{fig:3DChannel_planes}(a)-\ref{fig:3DChannel_sat}(a) and \ref{fig:3DChannel_planes}(b)-\ref{fig:3DChannel_sat}(f) as they both describe the darcy velocity contours and the saturation front for the same timesteps, is necesserary. As the flow progress, in a $2D$ domain a finger formation will start to appear. This is triggerded initially cause of the permeability differences in the domain. In a $3D$ domain though, the same finger fomration does not look the same, as it is more a description of the preferential flow path that includes smaller fingers that will break out of the flow path. Two characteristic time steps are shown in figures \ref{fig:3DChannel_sat}(a)-(f), along with the evolution of the saturation front that now becomes more obvious. Based on the boundary conditions that implemented in this domain, the preferencial path of the flow can been seen. In a large scale enhanced oil-recovery operation this might have direct impact on the in-sity hydrocarbons located in lower permeability areas. 
