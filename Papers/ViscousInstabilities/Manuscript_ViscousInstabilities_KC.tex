...
This example serves as a base case for comparison between mobility ratios that differ by an order of magnitude. In general for mobility ratios below $5$ no fingers are expexted as the injection fron will uniformly develop as time progress. Instabilities will start to occur for mobility ratios higher than $5$. Fingers or accelrated flows will be triggered from permeability differences on the top layers of the domain and the front will start to collapse. The interface between the two fluid will become unstable, an uneven or fingered profile will be formed due to capilary pressure and therefore due to the surface tension difference between the two immiscible fluids. Concequantly the productivity of an oil field will be affected and will decrease.
...
\end{center}
\caption{Comparing MR1 and MR10 cases using fixed mesh.}
\label{fig:}
\end{figure}
