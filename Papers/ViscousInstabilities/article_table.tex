...
\begin{table}
  \begin{tabular}{c | c c  c  c  c  c  c  c  c  c   c}
    ...
    \hline
     ...
     \hline
   \end{tabular}
   \caption{Sumary of model set-up used in the numerical simulations. Superscript $\circ$ denotes initial condition. $\mathbf{K}_{i}$ is in $cm^{2}$ and $u^{\circ}_{w}$ is in m/sec. $S_{w,irr}$ and $S_{nw,r}$ are the same for all simulations. \red{(HAVE YOU UPDATED THIS TABLE ???)} }
\label{table:setup}
\end{table}
\end{landscape}
\clearpage
