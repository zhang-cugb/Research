...
\bigskip
This work focuses on reducing the order of permeability field distribution for multiphase flow simulations in porous media. Section~\ref{section:mathematical_model} presents the mathematical model used to describe the relevant physics for multi-fluid flow through porous media and highlights the relevance of the permeability field. The mathematical model is discretised by a novel high-order accurate control volume finite element method (CVFEM) which couples both the fine resolution and upscaled representation of the permeability field. Pre-processing statistical properties and post-simulation multiphase flow behaviour are investigated to analyse the various upscaling techniques used.
...
Section~\ref{section:intro} briefly introduced the rational behind upscaling. Upscaling methods replace discrete geological and fluid properties of detailed high resolution domains with coarse descriptions (low resolution) of these properties \cite{Vereecken_2007}. This is a subject which has interested the academic community for over $7$ decades.  \citet{Cardwell_1945} investigated arithmetic and weighted averages as upscaling techniques for calculating a single equivalent permeability for an oil reservoir with varying permeabilities. \citet{King1996} investigated the use of renormalization for upscaling. And \citet{Yeo2001} discussed the accuracy of renormalization method for upscaling 2D hydraulic conductivity fields. \citet{Renard_1997} provided a comprehensive review of the various techniques for upscaling. \citet{Christie_2001SPE10Model} provided $2$ sets of problems to compare the upscaling and upgridding techniques of different simulators. \red{(And what are the main conclusions/contributions of these papers?)}

\JGnote{You should consider writing a more detailed section on upscaling as this is part of the core of your work here ...}

\BLnote{I will do this before submitting, please let me know if you will be able to review again on Thursday or Friday}
...
\end{equation}
\red{(I guess we have talked about this on Friday ... make it clear that matrix inversion is an (useful?) example of SVD ... also explain step-by-step this matricial equations above ... remember ... every single equation must be explained ...)}

\BLnote{The equations are now explained in the last 3 sentences of this section.}



\subsection{Principal Component Spaces and Model Order Reduction for the permeability field}\label{subsection:pcspaces}
\JGnote{As we talked on Friday: (a)make sure you explain what permeability is before starting to operating on it; (b) $P0DG$ could be easily replaced by saying that the quantity of interest is allocated in the centre of the element/cell -- this make it more general; (c) Just reading it through, we can summarise the {\it procedure} as (i) SVD of matrix of permeability scalar quantities (\ie upscaling) and (ii) linear interpolation of the upscaled/homogeneised quantities upon a new mesh grid resolution (\ie upgriding). Whereas the first is the object of the interest for this study, the second gives an overall motivation for the work ... thus, make sure this is clear within the text, \ie upscaling $\rightarrow$ upgriding.}

\BLnote{(a)Permeability is now introduced in Section~\ref{section:mathematical_model}; (b) $P0DG$ now replaced with centre of element/cell (c) I thought we agreed that upscaling cannot be separated from upgridding. Actually, you convinced me that upscaling cannot be seperated from upgridding. Upscaling is not completed in the SVDCase until the interpolation is completed. Strictly speaking, interpolation only defines the level of upgridding. I will mention this in the results section (Section~\ref{section:results_discussion})}
...
\JGnote{Maybe you should consider describing the procedure above as pseudocode within a table.}

\BLnote{I have an algorithm in Section~\ref{subsubsection:svdcase_preprocess_algorithm} which itemizes the procedure in a sort of psuedocode}
