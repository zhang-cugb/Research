...
Mesh resolution for the the BaseCase and upscaled cases are shown in Fig,~\ref{fig:HiRes_LowRes_Mesh}. Domain of the BaseCase (\ie high-resolution mesh) was discretised with $4112$ \PNDG[1]{1} \red{(IS THIS TRUE?)} triangular element-pairs, whilst in upscaled cases the domain was discretised with $728$ \PNDG[1]{1} \red{(IS THIS TRUE?)} triangular element-pairs. Therefore the resolution for the upscaled cases is reduced by a factor of approximately $5.6$.

Figure~\ref{fig:PermFields} compares the permeability distribution for all models. \red{Permeability legend in Fig.~\ref{fig:PermFields}(a) is representative for all the other figures (\ie Fig.~\ref{fig:PermFields}(b) to (f))} \red{(THIS INFO SHOULD BE IN THE CAPTION OF THE FIGURES INSTEAD)}. The cases are shown without the edges for elements which discretize the domain in Fig.~\ref{fig:PermFields}(b) to (f).
...
\medskip
\red{(I GUESS YOU SHOULD USE ONE PARAGRAPH JUST TO DESCRIBE (TO THE READER WHO HASNT READ THE OTHER REFERENCES) THE PHYSICS ASSOCIATED WITH THE BASECASE -- FIG 4 LINKING WITH THE REFERENCES ... REMEMBER THIS IS YOUR WORK'S VALIDATION!!! THIS BENCHMARK WOULD: (A) IF THE SIMULATION WAS PERFORMED W HOMOGENEOUS MEDIA (AT EACH BLOCK) A PISTON SHAPE BEHAVIOR (AT EACH BLOCK, I.E., A BLOCK DIVIDED BUCKLEY-LEVERETT BEHAVIOR) COULD BE NOTICED HOWEVER AS THE WHOLE SYSTEM IS HETEROGENEOUS THIS MAY NOT BE CLEARLY NOTICED; (B) CROSS-FLOW, I.E., PREFERENTIAL FLOW  THROUGH REGIONS OF HIGH PERMEABILITY; (C) FLOW QUALITATIVE BEHAVIOUR MATCHES THE EXPERIMENTS  ETC ETC ETC MAKE IT STRONG !)}
