...
Furthermore, within each block, permeability dataset varies within a pre-defined range and consists of $1600$ permeability values. More details on the design of the permeability field for the BaseCase is described in Section~\ref{subsubsection:basecase_preprocess_algorithm}. Figures~\ref{fig:PermFields}a-b are pictorial representations of the permeability dataset for the base case with the mesh discretization visible and invisible, respectively.  Using the labelling scheme used by \citet{dawe_2008}, the quadrant model block $K1,~K2,~K3$ and $K4$ are top left, bottom left, top right and bottom right, respectively.
