...
\begin{description}
  \item[Step 1:] In {\it Step 1} of Section~\ref{subsubsection:svdcase_preprocess_algorithm}, HOSVD method is used for factorization instead of SVD. Therefore, a $3 \times 3$ core tensor is obtained in place of the $\mathbf{\Sigma}$ term, while 3 factor matrices are obtained in place of $\mathbf{U}$ and $\mathbf{V}$;
  \item[Step 2:] The core tensor is treated as the $\mathbf{\Sigma}$ term, while the 3 factor matrices are treated similarly to $\mathbf{U}$ and $\mathbf{V}$ as described in Section~\ref{subsubsection:svdcase_preprocess_algorithm}.
\end{description}
